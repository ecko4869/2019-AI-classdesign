...
for k in range(max_steps):
    ...
    for i in range(x_size):
        ...
        for j in range(hidesize):
            ...
            # print(j,sigmoid(j))
            hide_out[j] = sigmoid(hide_in[j],1)
        ...
        for j in range(hidesize):
            dB1[j] = np.dot(np.dot(W2[0][j], sigmoid(hide_in[j],1)), (1 - sigmoid(hide_in[j],1)) * (-1) * e * threshold)

        dW1 = np.zeros((hidesize, 1))

        for j in range(hidesize):
            dW1[j] = np.dot(np.dot(W2[0][j], sigmoid(hide_in[j],1)), (1 - sigmoid(hide_in[j],1)) * x[i] * e * threshold)
